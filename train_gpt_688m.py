#!/usr/bin/env python3
"""
HYPERCLONED 688M MODEL TRAINING

Expanded from 268M -> 688M using HyperCloning technique
Source: "Scaling Smart: Accelerating Large Language Model Pre-training with Small Model Initialization"

Architecture:
- 12 layers (same as source)
- 12 heads (expanded from 6)
- 1536 hidden dim (expanded from 768)
- 128 head dim (unchanged)

Training modifications:
- Conservative learning rates (model is warm-initialized)
- Shorter training schedule (2500 steps vs 1750)
- Short warmup (100 steps vs 0)
...
- Custom architecture: smear_gate, backout_lambda, skip connections, value embeds
"""
import os
import sys
...
class GPTConfig:
    ...
    n_layer: int = 12
    n_head: int = 12  # EXPANDED: head dim 128 (was 6)
    n_embd: int = 1536  # EXPANDED: 2x width (was 768)
...
class Hyperparameters:
    ...
    batch_size: int = 8  # REDUCED for 688M model (was 16)
    sequence_length: int = 8 * 1024  # REDUCED: 8K context for memory (was 32K)
    num_iterations: int = 2500  # HYPERCLONED: Shorter than scratch (was 1750)
    warmup_iters: int = 100  # HYPERCLONED: Short warmup (was 0)
...
# Load checkpoint (resume or HyperCloned init)
resume_checkpoint = ""  # Disabled - checkpoint corrupted from disk full
hypercloned_checkpoint = "gpt2-300m-hypercloned-init.pt"
start_step = 0

if os.path.exists(resume_checkpoint):
    print0(f"\n{'='*70}")
    print0(f"RESUMING FROM CHECKPOINT")
    print0(f"{'='*70}")
    ckpt = torch.load(resume_checkpoint, weights_only=False, map_location="cpu")
    ...
    print0(f"  Previous loss: {ckpt.get('val_loss', 'N/A')}")
    print0(f"Checkpoint loaded successfully!")
    print0(f"{'='*70}\n")

elif os.path.exists(hypercloned_checkpoint):
    print0(f"\n{'='*70}")
    print0(f"LOADING HYPERCLONED CHECKPOINT")
    print0(f"{'='*70}")
    ckpt = torch.load(hypercloned_checkpoint, weights_only=False, map_location="cpu")

    # Load the expanded model weights
    state_dict = {}
    for k, v in ckpt['model'].items():
        new_k = k.replace("_orig_mod.", "")
        state_dict[new_k] = v
    ...
    print0(f"  Source checkpoint: {ckpt['source_checkpoint']}")
    print0(f"  Source step: {ckpt.get('source_step', 'N/A')}")
    print0(f"  Source loss: {ckpt.get('source_loss', 'N/A')}")
    print0(f"  Expansion factor: {ckpt['expansion_factor']}x")
    print0(f"HyperCloned model loaded successfully!")
    print0(f"{'='*70}\n")
else:
    print0(f"WARNING: No checkpoint found")
    print0("Training from random initialization instead")

# IMPROVED: Separate optimizers with different learning rates
# HYPERCLONED: More conservative LRs since model is warm-initialized
# Original: wte=0.6, lm_head=0.008, Muon=0.05, scalars=0.04
# HyperCloned: wte=0.4, lm_head=0.005, Muon=0.03, scalars=0.03
optimizer1 = torch.optim.Adam([model.transformer.wte.weight], lr=0.4, betas=(0.8, 0.95), fused=True)
for ve in model.value_embeds:
    optimizer1.add_param_group({'params': [ve.weight], 'lr': 0.4})

optimizer2 = torch.optim.Adam([model.lm_head.weight], lr=0.005, betas=(0.8, 0.95), fused=True)
...
optimizer3 = Muon(matrix_params, lr=0.03, momentum=0.95)
optimizer4 = torch.optim.Adam(scalar_params, lr=0.03, betas=(0.8, 0.95), fused=True)
